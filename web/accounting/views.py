...
class PurchaseOwnerPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.method == 'PATCH':
            ...
            if not perm_method.request_param_validation(purchase, forbidden_queries, request.data):
                return False
...
class ProjectPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        allowed_perm_classes = [AdminPermission, ReadOnlyPermission, ProjectOwnerPermission]
...
class PurchasePermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        allowed_perm_classes = [AdminPermission, ReadOnlyPermission, PurchaseOwnerPermission]
        return any(perm_class().has_permission(request, view) for perm_class in allowed_perm_classes)
