...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    def get_sum_budget(self, obj):
        query_result = ProjectApproval.objects.filter(project_id=obj.id, approved=True).aggregate(Sum('budget_amount'))['budget_amount__sum']
...
class ProjectDetailSerializer(ProjectSerializer):
    ...
    class Meta:
        model = Project
        fields = ('id', 'title', 'accounting_type', 'leader', 'closed', 'sum_budget', 'sum_req_budget', 'sum_purchase_price', 'approvals', 'purchases')
